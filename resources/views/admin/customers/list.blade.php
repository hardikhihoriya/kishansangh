...
<section class="content-header">
    <h1>
        ...
        <div class="pull-right">
            <a href="{{ url('admin/customer/new') }}" class="btn btn-default pull-right-responsive"><span><i class="fa fa-plus-circle"></i>&nbsp;&nbsp;</span>{{trans('adminlabels.ADD_CUSTOMER')}}</a>
        </div>       
    </h1>
</section>
        ...
        <div class="col-md-12">
            <div class="box">
                <div class="box-header">
                    <h3 class="box-title">{{trans('adminlabels.CUSTOMER_LIST')}}</h3>
                </div>
                    ...
                    <table id="listCustomer" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                ...
                                <th>{{trans('adminlabels.CUSTOMER_WALLET')}}</th>
                                <th>{{trans('adminlabels.NOMINEE_PHOTO')}}</th>
...
<script>
    var getCustomerList = function (ajaxParams) {
        $('#listCustomer').DataTable({
            "processing": true,
            "serverSide": true,
            "destroy": true,
            "ajax": {
                "url": "{{ url('admin/customer/list-ajax') }}",
                "dataType": "json",
                "type": "POST",
                headers: {
            ...
            "columns": [
                ...
                {"data": "nominee_photo", "orderable": false},
    ...
    $(document).ready(function () {
        var ajaxParams = {};
        getCustomerList(ajaxParams);

        // Remove user
        $(document).on('click', '.btn-delete-customer', function (e) {
            e.preventDefault();
            var customerId = $(this).attr('data-id');
            var cmessage = 'Are you sure you want to Delete this Customer ?';
            var ctitle = 'Delete Customer';

            ajaxParams.customActionType = 'groupAction';
            ajaxParams.customActionName = 'delete';
            ajaxParams.id = [customerId];
